...
export const updateQuiz = async (req, res) => {
  try {
    ...
    const updatedQuiz = await Quiz.findByIdAndUpdate(req.params.id, { title, subject, questions }, { new: true });
    ...
    console.error("Error in updateQuiz controller: ", error.message)
    res.status(500).json({ message: "Server Error", error: error.message });
  }
}
